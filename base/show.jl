    ...
    for item in items
        ...
        parens = !is_quoted(item) &&
            (first && prec >= prec_power &&
             ((item isa Expr && item.head === :call && (callee = item.args[1]; isa(callee, Symbol) && callee::Symbol in uni_ops)) ||
...
function show_unquoted(io::IO, ex::Expr, indent::Int, prec::Int, quote_level::Int = 0)
    ...
    elseif head === :call && nargs >= 1
        ...
        elseif func_prec > 0 # is a binary operator
            na = length(func_args)
            if (na == 2 || (na > 2 && isa(func, Symbol) && func::Symbol in (:+, :++, :*)) || (na == 3 && func === :(:))) &&
    ...
    elseif head === :do && nargs == 2
        ...
        print(io, " do ")
        dovars = (((args[2]::Expr).args[1])::Expr).args
        show_list(iob, dovars, ", ", 0, 0, quote_level)
        for stmt in dovars
            print(io, '\n', " "^(indent + indent_width))
    ...
    elseif (head === :if || head === :elseif) && nargs == 3
        ...
        if isa(args[3],Expr) && (args[3]::Expr).head === :elseif
            show_unquoted(iob, args[3]::Expr, indent, prec, quote_level)
        else
